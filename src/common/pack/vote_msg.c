                                ...
                                char** msg)
{
    ...
    msgpack_pack_uint64(&pk, data->candidate_term);
    msgpack_pack_uint64(&pk, data->last_log_index);
    msgpack_pack_uint64(&pk, data->last_log_term);

    // TODO: pack the candidate uuid
                                  ...
                                  char* msg)
{
    ...
    if(msgpack_unpack_next(&unpacked, msg, len, NULL)) {
        ...
        if(root.type == MSGPACK_OBJECT_ARRAY && root.via.array.size == 6) {
            ...
            data->candidate_term = root.via.array.ptr[1].via.u64;
            data->last_log_index = root.via.array.ptr[2].via.u64;
            data->last_log_term = root.via.array.ptr[3].via.u64;

            //TODO: unpack candidate uuid
        }
    }
                                     ...
                                     char** msg)
{
    ...
    msgpack_pack_uint8(&pck, data->vote_granted);
